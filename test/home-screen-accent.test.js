...
test("home-screen ad CTA renders as a real glowing capsule and Settings keeps its glow", async () => {
  ...
  assert.match(start, /\/home-screen-accent\.js\?v=20260911-3/);
  assert.match(source, /keiazo-ad-pill-overlay/);
  assert.match(source, /Advertise your business · \$/);
  assert.match(source, /currentTopBidDollars/);
  assert.match(source, /stroke-linecap: round/);
  assert.match(source, /keiazo-ad-pill-rim/);
  assert.match(source, /targetTotalWidth = 198 \* scale/);
  assert.match(source, /visualHeight = 28 \* scale/);
  assert.match(source, /@keyframes keiazoAdPillGlow/);
